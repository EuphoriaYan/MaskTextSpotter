    ...
    for i, batch in tqdm(enumerate(data_loader)):
        ...
        with torch.no_grad():
            ...
            if predictions is not None:
                ...
                global_predictions = [o.to(cpu_device) for o in global_predictions]
                results_dict.update(
                    {image_paths[0]: [global_predictions[0], char_mask, boxes, seq_words, seq_scores,
                                      detailed_seq_scores]}
                )
    return results_dict
...
def format_output(out_dir, boxes, img_name):
    ...
    for i, box in enumerate(boxes):
        ...
        if 'total_text' in out_dir or 'cute80' in out_dir:
            ...
            box = ','.join([str(x) for x in box[:4]]) + ';' + ','.join(
                [str(x) for x in box[4:4 + int(box[-1])]]) + ';' + ','.join(
                [str(x) for x in box[4 + int(box[-1]):-3]]) + ',' + save_name
        else:
