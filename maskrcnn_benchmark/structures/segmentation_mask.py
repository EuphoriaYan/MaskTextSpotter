...
class CharPolygons(object):
    ...
    def transpose(self, method):
        ...
        return CharPolygons(flipped_polygons, word=self.word, use_char_ann=self.use_char_ann,
                            char_classes=self.char_classes, size=self.size, mode=self.mode)
    ...
    def crop(self, box):
        ...
        return CharPolygons(cropped_polygons, word=self.word, use_char_ann=self.use_char_ann,
                            char_classes=self.char_classes, size=(w, h), mode=self.mode)
    ...
    def rotate(self, angle, r_c, start_h, start_w):
        ...
        return CharPolygons(r_polys, word=self.word, use_char_ann=self.use_char_ann, char_classes=self.char_classes,
                            size=(r_c[0] * 2, r_c[1] * 2), mode=self.mode)
    ...
    def resize(self, size, *args, **kwargs):
        ...
        if ratios[0] == ratios[1]:
            ...
            return CharPolygons(scaled_polys, word=self.word, use_char_ann=self.use_char_ann,
                                char_classes=self.char_classes, size=size, mode=self.mode)
        ...
        return CharPolygons(scaled_polygons, word=self.word, use_char_ann=self.use_char_ann,
                            char_classes=self.char_classes, size=size, mode=self.mode)
        ...
        if mode == "char_mask":
            ...
            if not self.use_char_ann:
                ...
                if pos_num > 0:
                    pos_weight = 1.0 * (height * width - pos_num) / pos_num
                    char_map_weight[1:] = pos_weight
            return torch.from_numpy(char_map), torch.from_numpy(char_map_weight), torch.from_numpy(
                decoder_target), torch.from_numpy(word_target)
...
class SegmentationCharMask(object):
    def __init__(self, chars_boxes, words=None, use_char_ann=True, size=None, mode=None):
        ...
        if words is None:
            self.chars_boxes = [CharPolygons(char_boxes, word=None, use_char_ann=use_char_ann, size=size, mode=mode) for
                                char_boxes in chars_boxes]
        else:
            self.chars_boxes = [CharPolygons(char_boxes, word=words[i], use_char_ann=use_char_ann, size=size, mode=mode)
    ...
    def rotate(self, angle, r_c, start_h, start_w):
        ...
        return SegmentationCharMask(rotated, use_char_ann=self.use_char_ann, size=(r_c[0] * 2, r_c[1] * 2),
                                    mode=self.mode)
    ...
    def __getitem__(self, item):
        ...
        return SegmentationCharMask(selected_chars_boxes, use_char_ann=self.use_char_ann, size=self.size,
                                    mode=self.mode)
